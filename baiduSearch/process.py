...
class Result(object):
    ...
    def __init__(self, r_index, r_title, r_abstract, r_url):  # id似乎占用了内部名称，那就用index来代替吧
...
def page(html):
    ...
    # 获取结果来源
    # print soup
    result_set = soup.find(id = "wgt-list")  # 结果全显示在页面左边

    result_set = result_set.find_all('dl', class_='dl')  # 结果class固定，其余为硬广
    ...
    for i in range(len(result_set)):  # 因为要index所以就用range来
        ...
        result = Result(i + 1, c_title, c_abstract, c_url)
        if c_abstract != '百度软广，当前代码版本不予摘要':
...
def __get_abstract(result_div):
    ...
    r_from = result_div.find(class_='dd answer')
    if not r_from:
        return None
